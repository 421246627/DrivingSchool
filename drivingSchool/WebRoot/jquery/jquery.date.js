(function (factory) {
    ...
    $('#d-confirm').bind('click',function(){
        ...
        if(opt.limitTime == 'today'){
            ...
            //当前日期
            var day = String(currentd.getFullYear())+'-'+String(currentd.getMonth() + 1)+'-'+String(currentd.getDate());
            ...
            error = !isEnglish ? '抱歉,只能预约十五天内的时间!':'Sorry, we can only book 15 days!';
            if(y>currentd.getFullYear())
            {
            	alert(error);
            	return false;
            }
